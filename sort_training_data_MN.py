...
CSV_IN  = "metalpdb_binding_windows_len10_MN.csv"
CSV_OUT = "metalpdb_binding_windows_len10_MN_scored_ranked.csv"
...
def compute_scores_mn_df(df_in: pd.DataFrame, reverse_translate_strategy="first") -> pd.DataFrame:
    df = df_in.copy()
    ...
    # a3d_means = {}
    # cnt = 0
    # a3d_out = "MN_metaldb_a3ds/"
    # pdb_path = "MN_metaldb_pdbs/peptide_"
    # for seq in uniq_seq:
    #     print(cnt)
    #     cnt = cnt+1
    #       pdb_out_dir = f"C:\\Users\\shima\\OneDrive\\Documentos\\Leili\\peptide_structure_optimization\\peptide_optimization\\MN_metaldb_pdbs\\peptide_{seq}\\input.fasta"
    #     pdb_path = run_colabfold_here(seq, out_dir=pdb_out_dir, threads=8)
    # # Run A3D 
    # cnt = 0
    # a3d_out = Path("MN_metaldb_a3ds")
    # pdb_root = Path("MN_metaldb_pdbs")

    # EXPECTED_CSV = "A3D.csv"  # what run_a3d_on_pdb writes/returns
    ...
    # if "a3d_mean" not in df.columns:
    #     df["a3d_mean"] = df["peptide_len10"].map(a3d_means)
    # ------------------------------------------------------------------
    # NEW: Load A3D.csv per peptide and compute mean(score)
    # ------------------------------------------------------------------
    A3D_ROOT = Path("MN_metaldb_a3ds")  # <-- change to MN_metaldb_a3d if needed
...
def main():
    ...
    df_out = compute_scores_mn_df(df_in)
