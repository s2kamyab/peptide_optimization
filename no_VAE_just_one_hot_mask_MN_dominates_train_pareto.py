...
TRAIN_PARETO_CSV = "bo_results\\train_pareto.csv"
OUT_DOMINATES_TRAIN = os.path.join(OUT_DIR, "bo_mo_noVAE_MN_dominates_train.csv")
OUT_DOMINATES_TRAIN_PARETO = os.path.join(OUT_DIR, "bo_mo_noVAE_MN_dominates_train_pareto.csv")
...
DominanceBoost = 2.0  # boost in screening for candidates predicted to dominate training Pareto
DOMINANCE_SCREEN_FRAC = 0.50  # reserve this fraction of SCREEN_K for dominance-focused candidates
...
def dominates_max(y: np.ndarray, ref: np.ndarray) -> bool:
    y = np.asarray(y, dtype=float)
    ref = np.asarray(ref, dtype=float)
    return bool(np.all(y >= ref) and np.any(y > ref))


def dominance_margin_to_set(y: np.ndarray, ref_set: np.ndarray) -> float:
    """
    For a candidate objective vector y, compute the best minimum coordinate-wise margin
    against any point in ref_set:
        score(y) = max_r min_m ( y_m - r_m )
    score > 0 means y strictly dominates at least one reference point.
    """
    y = np.asarray(y, dtype=float)
    ref_set = np.asarray(ref_set, dtype=float)
    if ref_set.size == 0:
        return -np.inf
    margins = y[None, :] - ref_set  # [R, M]
    return float(margins.min(axis=1).max())


def rows_dominating_reference(df: pd.DataFrame, ref_Y: np.ndarray) -> pd.DataFrame:
    obj_cols = ["obj_chelation", "obj_solubility", "obj_stability", "obj_expression"]
    if df.empty:
        return df.copy()
    Y = df[obj_cols].to_numpy(dtype=float)
    keep = np.array([dominance_margin_to_set(y, ref_Y) > 0.0 for y in Y], dtype=bool)
    out = df.loc[keep].copy()
    if not out.empty:
        out["dominates_train_margin"] = [
            dominance_margin_to_set(y, ref_Y) for y in out[obj_cols].to_numpy(dtype=float)
        ]
    return out.reset_index(drop=True)


def load_train_pareto(csv_path: str) -> pd.DataFrame:
    df = pd.read_csv(csv_path).copy()
    rename = {
        "chelation_sub": "obj_chelation",
        "solubility_sub": "obj_solubility",
        "stability_sub": "obj_stability",
        "expression_sub": "obj_expression",
    }
    df = df.rename(columns=rename)
    needed = ["peptide", "obj_chelation", "obj_solubility", "obj_stability", "obj_expression"]
    missing = [c for c in needed if c not in df.columns]
    if missing:
        raise ValueError(f"Training Pareto CSV missing columns: {missing}")
    df["peptide"] = df["peptide"].astype(str).map(clean_seq)
...
def mo_bo_no_vae_fast(
    ...
    Returns:
      ...
      df_dom_train: evaluated points that dominate at least one training Pareto point
      df_dom_train_pareto: nondominated subset within df_dom_train
    """
    ...
    train_pareto_Y = train_pareto_df[
    ...
    for t in range(n_steps):
        ...
        # ---- SCREENING: cheap proxy + dominance-to-training-Pareto bias
        with torch.no_grad():
            means = []
            for j in range(4):
                pj = model_mo.models[j].posterior(X_cand).mean.squeeze(-1)  # (n,)
                means.append(pj)
            mean_mat = torch.stack(means, dim=1)  # (n,4)
            mean_scalar = mean_mat.mean(dim=1)  # (n,)
            mean_np = mean_mat.detach().cpu().numpy()
            dom_margin = np.array(
                [dominance_margin_to_set(y, train_pareto_Y) for y in mean_np],
                dtype=float,
            )
            dom_mask = dom_margin > 0.0
            boosted_score = mean_scalar.detach().cpu().numpy() + DominanceBoost * np.maximum(dom_margin, 0.0)

        k = min(SCREEN_K, X_cand.shape[0])
        k_dom = min(int(max(1, round(DOMINANCE_SCREEN_FRAC * k))), int(dom_mask.sum()))
        chosen = []

        if k_dom > 0:
            dom_idx = np.where(dom_mask)[0]
            dom_pick_local = dom_idx[np.argsort(-boosted_score[dom_idx])[:k_dom]]
            chosen.extend(dom_pick_local.tolist())

        remaining = k - len(chosen)
        if remaining > 0:
            all_order = np.argsort(-boosted_score)
            for idx0 in all_order:
                if idx0 not in chosen:
                    chosen.append(int(idx0))
                if len(chosen) >= k:
                    break

        top_idx_np = np.array(chosen, dtype=int)
        top_idx = torch.tensor(top_idx_np, dtype=torch.long, device=DEVICE)
        X_small = X_cand[top_idx]
        cand_small = [cand_peps[i] for i in top_idx_np.tolist()]
        ...
        dom_new = np.array([dominance_margin_to_set(y, train_pareto_Y) > 0.0 for y in new_Y_np], dtype=bool)
        ...
        if (t + 1) % 5 == 0:
            ...
            n_dom_train = int(rows_dominating_reference(
                pd.DataFrame([r.__dict__ for r in log_rows]), train_pareto_Y
            ).shape[0])
            print(
                f"[step {t+1:03d}/{n_steps}] evaluated={Y.shape[0]} "
                f"pareto={int(nd.sum())} dominates_train={n_dom_train} "
                f"last_batch={peps_next} new_dom={int(dom_new.sum())}"
            )

    # outputs
    df_log = pd.DataFrame([r.__dict__ for r in log_rows])
    obj_cols = ["obj_chelation", "obj_solubility", "obj_stability", "obj_expression"]
    Y_all = df_log[obj_cols].to_numpy(dtype=float)
    nd_mask = is_non_dominated_max(Y_all)
    df_pareto = df_log.loc[nd_mask].reset_index(drop=True)

    df_dom_train = rows_dominating_reference(df_log, train_pareto_Y)
    if not df_dom_train.empty:
        nd_dom = is_non_dominated_max(df_dom_train[obj_cols].to_numpy(dtype=float))
        df_dom_train_pareto = df_dom_train.loc[nd_dom].reset_index(drop=True)
    else:
        df_dom_train_pareto = df_dom_train.copy()

    return df_log, df_pareto, df_dom_train, df_dom_train_pareto


def main():
    df = pd.read_csv(CSV_DATASET).dropna(subset=[SEQ_COL]).copy()
    df[SEQ_COL] = df[SEQ_COL].astype(str).map(clean_seq)
    train_pareto_df = load_train_pareto(TRAIN_PARETO_CSV)
    ...
    df_log, df_pareto, df_dom_train, df_dom_train_pareto = mo_bo_no_vae_fast(
        df,
        ref_candidates=ref,
        train_pareto_df=train_pareto_df,
        binding_sites="",
        n_init=N_INIT,
        n_steps=N_STEPS,
    )
    ...
    df_dom_train.to_csv(OUT_DOMINATES_TRAIN, index=False)
    df_dom_train_pareto.to_csv(OUT_DOMINATES_TRAIN_PARETO, index=False)

    print(f"\nSaved full log -> {OUT_LOG}   (rows={len(df_log)})")
    print(f"Saved Pareto front -> {OUT_PARETO}   (rows={len(df_pareto)})")
    print(f"Saved points dominating training Pareto -> {OUT_DOMINATES_TRAIN}   (rows={len(df_dom_train)})")
    print(f"Saved nondominated subset of dominating points -> {OUT_DOMINATES_TRAIN_PARETO}   (rows={len(df_dom_train_pareto)})")
    print("\nTop Pareto (first 10):")
    print(df_pareto.head(10))
    print("\nTop points dominating training Pareto (first 10):")
    print(df_dom_train_pareto.head(10))
