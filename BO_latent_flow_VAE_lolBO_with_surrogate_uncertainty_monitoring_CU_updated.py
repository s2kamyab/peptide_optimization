...
# ---- Surrogate uncertainty/error monitoring ----
# These diagnostics compare the MO-GP surrogate prediction against the
# black-box objective values for BO-evaluated decoded peptides.
SURROGATE_MONITOR_DIR = "bo_surrogate_monitoring"
...
# ============================================================
# Surrogate uncertainty / error diagnostics for BO candidates
# ============================================================

def _posterior_mean_std(posterior) -> Tuple[torch.Tensor, torch.Tensor]:
    """Return posterior mean/std as [N, M] tensors for BoTorch ModelListGP."""
    pred_mean = posterior.mean
    pred_std = posterior.variance.clamp_min(0.0).sqrt()

    # Depending on BoTorch version/model shape, outputs may be [N, M] or [N, 1, M].
    if pred_mean.dim() == 3 and pred_mean.size(-2) == 1:
        pred_mean = pred_mean.squeeze(-2)
    if pred_std.dim() == 3 and pred_std.size(-2) == 1:
        pred_std = pred_std.squeeze(-2)

    return pred_mean, pred_std


def _categorize_uncertainty_error(mean_uncertainty: float, mean_abs_error: float,
                                  uncertainty_threshold: float, error_threshold: float) -> str:
    """Human-readable quadrant label for uncertainty-vs-error analysis."""
    uncertainty_level = "high_uncertainty" if mean_uncertainty >= uncertainty_threshold else "low_uncertainty"
    error_level = "high_error" if mean_abs_error >= error_threshold else "low_error"

    if uncertainty_level == "low_uncertainty" and error_level == "low_error":
        return "reliable_low_uncertainty_low_error"
    if uncertainty_level == "high_uncertainty" and error_level == "low_error":
        return "cautious_high_uncertainty_low_error"
    if uncertainty_level == "high_uncertainty" and error_level == "high_error":
        return "difficult_high_uncertainty_high_error"
    return "overconfident_low_uncertainty_high_error"


def plot_surrogate_uncertainty_error(df_diag: pd.DataFrame, out_dir: str, bo_iter: int) -> Dict[str, str]:
    """Save uncertainty/error scatter and histograms for one BO iteration."""
    _ensure_dir(out_dir)
    paths = {}
    ...
    uncertainty_threshold = float(df_diag["mean_uncertainty"].median())
    error_threshold = float(df_diag["mean_abs_error"].median())

    # Main scatter: x = uncertainty, y = error
    fig, ax = plt.subplots(figsize=(7.5, 5.5))
    ax.scatter(df_diag["mean_uncertainty"], df_diag["mean_abs_error"], s=35, alpha=0.8)
    ax.axvline(uncertainty_threshold, linestyle="--", linewidth=1, alpha=0.7)
    ax.axhline(error_threshold, linestyle="--", linewidth=1, alpha=0.7)
    ax.set_xlabel("Mean surrogate uncertainty across objectives")
    ax.set_ylabel("Mean absolute prediction error across objectives")
    ax.set_title(f"Surrogate uncertainty vs. prediction error — BO iter {bo_iter:03d}")
    ax.grid(True, alpha=0.3)

    # Annotate peptides lightly if there are not too many.
    if len(df_diag) <= 20:
        for _, row in df_diag.iterrows():
            ax.annotate(str(row["peptide"]),
                        (row["mean_uncertainty"], row["mean_abs_error"]),
                        fontsize=7, alpha=0.75, xytext=(3, 3), textcoords="offset points")

    scatter_path = os.path.join(out_dir, f"surrogate_uncertainty_vs_error_bo_iter_{bo_iter:03d}.png")
    fig.tight_layout()
    fig.savefig(scatter_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    paths["scatter"] = scatter_path

    # Mean uncertainty histogram
    fig, ax = plt.subplots(figsize=(6.5, 4.5))
    ax.hist(df_diag["mean_uncertainty"], bins=min(20, max(5, len(df_diag))))
    ax.set_xlabel("Mean surrogate uncertainty")
    ax.set_ylabel("Count")
    ax.set_title(f"Surrogate uncertainty distribution — BO iter {bo_iter:03d}")
    fig.tight_layout()
    uncertainty_hist_path = os.path.join(out_dir, f"surrogate_uncertainty_hist_bo_iter_{bo_iter:03d}.png")
    fig.savefig(uncertainty_hist_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    paths["uncertainty_hist"] = uncertainty_hist_path

    # Mean absolute error histogram
    fig, ax = plt.subplots(figsize=(6.5, 4.5))
    ax.hist(df_diag["mean_abs_error"], bins=min(20, max(5, len(df_diag))))
    ax.set_xlabel("Mean absolute prediction error")
    ax.set_ylabel("Count")
    ax.set_title(f"Surrogate prediction error distribution — BO iter {bo_iter:03d}")
    fig.tight_layout()
    error_hist_path = os.path.join(out_dir, f"surrogate_error_hist_bo_iter_{bo_iter:03d}.png")
    fig.savefig(error_hist_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    paths["error_hist"] = error_hist_path

    return paths


@torch.no_grad()
def save_surrogate_uncertainty_diagnostics(
    model: FlowGRUVAE,
    mo_model: ModelListGP,
    cand_peps: List[str],
    Y_new: torch.Tensor,
    z_to_unit,
    bo_iter: int,
    out_dir: str = SURROGATE_MONITOR_DIR,
) -> pd.DataFrame:
    """
    Compare MO-GP predictions with black-box scores for decoded BO candidates.

    Important: because one acquisition latent point can decode into multiple peptides,
    this diagnostic re-encodes each accepted peptide and evaluates the surrogate at
    the encoded candidate location. This aligns the surrogate prediction with the
    actual sequence sent to the black-box evaluator.
    """
    _ensure_dir(out_dir)

    if len(cand_peps) == 0:
        empty = pd.DataFrame()
        empty.to_csv(os.path.join(out_dir, f"bo_uncertainty_error_iter_{bo_iter:03d}.csv"), index=False)
        return empty

    model.eval()
    mo_model.eval()

    X_cand = onehot_encode_peptides(cand_peps).to(DEVICE)
    Z_cand_encoded = encode_to_latent_mu(model, X_cand).detach().double().to(DEVICE)
    U_cand_encoded = z_to_unit(Z_cand_encoded).to(DEVICE)

    posterior = mo_model.posterior(U_cand_encoded)
    pred_mean, pred_std = _posterior_mean_std(posterior)

    pred_mean = pred_mean.detach().double().to(DEVICE)
    pred_std = pred_std.detach().double().to(DEVICE)
    Y_true = Y_new.detach().double().to(DEVICE)

    # Defensive shape check: expected [N, 4].
    if pred_mean.shape != Y_true.shape:
        raise RuntimeError(
            f"Surrogate diagnostic shape mismatch: pred_mean={tuple(pred_mean.shape)}, "
            f"Y_true={tuple(Y_true.shape)}."
        )

    abs_error = torch.abs(pred_mean - Y_true)
    mean_abs_error = abs_error.mean(dim=1)
    mean_uncertainty = pred_std.mean(dim=1)

    # Thresholds define quadrants inside the current iteration's candidates.
    uncertainty_threshold = float(mean_uncertainty.median().item())
    error_threshold = float(mean_abs_error.median().item())

    diagnostic_rows = []
    for i, pep in enumerate(cand_peps):
        row = {
            "bo_iter": int(bo_iter),
            "peptide": pep,

            "true_chelation": float(Y_true[i, 0].item()),
            "true_solubility": float(Y_true[i, 1].item()),
            "true_stability": float(Y_true[i, 2].item()),
            "true_expression": float(Y_true[i, 3].item()),

            "pred_chelation": float(pred_mean[i, 0].item()),
            "pred_solubility": float(pred_mean[i, 1].item()),
            "pred_stability": float(pred_mean[i, 2].item()),
            "pred_expression": float(pred_mean[i, 3].item()),

            "std_chelation": float(pred_std[i, 0].item()),
            "std_solubility": float(pred_std[i, 1].item()),
            "std_stability": float(pred_std[i, 2].item()),
            "std_expression": float(pred_std[i, 3].item()),

            "abs_error_chelation": float(abs_error[i, 0].item()),
            "abs_error_solubility": float(abs_error[i, 1].item()),
            "abs_error_stability": float(abs_error[i, 2].item()),
            "abs_error_expression": float(abs_error[i, 3].item()),

            "mean_abs_error": float(mean_abs_error[i].item()),
            "mean_uncertainty": float(mean_uncertainty[i].item()),
            "uncertainty_threshold_median": uncertainty_threshold,
            "error_threshold_median": error_threshold,
        }
        row["uncertainty_error_region"] = _categorize_uncertainty_error(
    ...
    csv_path = os.path.join(out_dir, f"bo_uncertainty_error_iter_{bo_iter:03d}.csv")
    df_diag.to_csv(csv_path, index=False)

    # Also save a sorted version that surfaces the most concerning candidates first.
    sorted_csv_path = os.path.join(out_dir, f"bo_uncertainty_error_ranked_iter_{bo_iter:03d}.csv")
    df_diag.sort_values(
        ["mean_abs_error", "mean_uncertainty"],
        ascending=[False, True],
    ).to_csv(sorted_csv_path, index=False)

    plot_paths = plot_surrogate_uncertainty_error(df_diag, out_dir=out_dir, bo_iter=bo_iter)

    # Lightweight W&B logging. Keep it robust so monitoring never kills BO.
    try:
        payload = {
            "bo_iter": bo_iter,
            "bo/surrogate_mean_abs_error": float(df_diag["mean_abs_error"].mean()),
            "bo/surrogate_max_abs_error": float(df_diag["mean_abs_error"].max()),
            "bo/surrogate_mean_uncertainty": float(df_diag["mean_uncertainty"].mean()),
            "bo/surrogate_max_uncertainty": float(df_diag["mean_uncertainty"].max()),
            "bo/surrogate_n_overconfident_wrong": int((df_diag["uncertainty_error_region"] == "overconfident_low_uncertainty_high_error").sum()),
            "bo/surrogate_uncertainty_error_table": wandb.Table(dataframe=df_diag),
        }
        if "scatter" in plot_paths:
            payload["bo/surrogate_uncertainty_vs_error_plot"] = wandb.Image(plot_paths["scatter"])
        wandb.log(payload)
    except Exception as e:
        try:
            wandb.log({"bo_iter": bo_iter, "bo/surrogate_monitoring_error": str(e)})
        except Exception:
            pass
...
def main():
    ...
    for bo_iter in range(BO_ITERS):
        ...
        # ---- Surrogate uncertainty/error monitoring ----
        # Compare the MO-GP surrogate prediction against the black-box scores
        # for the actual accepted decoded peptides. CSVs and plots are saved
        # under SURROGATE_MONITOR_DIR for every BO iteration.
        try:
            surrogate_diag_df = save_surrogate_uncertainty_diagnostics(
                model=model,
                mo_model=mo_model,
                cand_peps=cand_peps,
                Y_new=Y_new,
                z_to_unit=z_to_unit,
                bo_iter=bo_iter,
                out_dir=SURROGATE_MONITOR_DIR,
            )
        except Exception as e:
            surrogate_diag_df = pd.DataFrame()
            print(f"[WARN] Surrogate uncertainty/error monitoring failed at BO iter {bo_iter}: {e}")
            try:
                wandb.log({"bo_iter": bo_iter, "bo/surrogate_monitoring_error": str(e)})
            except Exception:
                pass
