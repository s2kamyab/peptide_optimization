...
# ---- Decoder monitoring knobs ----
DECODER_MONITOR_DIR = "bo_decoder_monitoring"
DECODER_SAMPLES_PER_Z = 8
DECODER_TEMPERATURE = 0.9
LOCAL_DECODING_MAX_Z = 5
LOCAL_DECODING_N_NEIGHBORS = 8
LOCAL_DECODING_NOISE_STD = 0.05
...
@torch.no_grad()
def _decode_logits_with_token_confidence(logits_2d: torch.Tensor, token_idx: Optional[torch.Tensor] = None) -> Tuple[str, float, float, List[float]]:
    """
    Convert decoder logits [L, V] into a peptide string and token-level confidence.

    If token_idx is None, uses argmax decoding. Otherwise, computes confidence for
    the provided sampled token indices.
    """
    probs = torch.softmax(logits_2d, dim=-1)  # [L, V]

    if token_idx is None:
        token_probs, idx = probs.max(dim=-1)
    else:
        idx = token_idx.to(device=logits_2d.device, dtype=torch.long)
        token_probs = probs.gather(-1, idx.unsqueeze(-1)).squeeze(-1)

    pep = "".join(I_TO_AA[int(i)] for i in idx.detach().cpu().tolist())
    token_probs_list = token_probs.detach().cpu().tolist()
    return (
        pep,
        float(token_probs.mean().item()),
        float(token_probs.min().item()),
        [float(v) for v in token_probs_list],
    )


@torch.no_grad()
def compute_roundtrip_latent_distance(
    model: FlowGRUVAE,
    peptide: str,
    ...
    Round-trip consistency:
        z_reference -> decoded peptide -> encoder mu

    Returns:
      - L2 distance ||z_reference - encode_mu(decoded_peptide)||
      - cosine similarity between z_reference and encode_mu(decoded_peptide)
    """
    x = onehot_encode_peptides([peptide]).to(z_reference.device)
    z_reconstructed = encode_to_latent_mu(model, x)[0].to(z_reference.device)

    z_ref = z_reference.reshape(-1)
    z_rec = z_reconstructed.reshape(-1)

    l2_dist = torch.norm(z_ref - z_rec, p=2).item()
    cos_sim = F.cosine_similarity(z_ref.unsqueeze(0), z_rec.unsqueeze(0), dim=-1).item()
    return float(l2_dist), float(cos_sim)
...
def decode_candidates_with_diagnostics(
    ...
    Z_cand: torch.Tensor,
    bo_iter: int,
    out_dir: str = DECODER_MONITOR_DIR,
    n_samples_per_z: int = DECODER_SAMPLES_PER_Z,
    temperature: float = DECODER_TEMPERATURE,
) -> Tuple[List[str], pd.DataFrame]:
    """
    Decode each optimized latent candidate into peptide proposals and save diagnostics.

    Diagnostics include:
      - decoder confidence = mean max/sample token probability
      - minimum token probability across the decoded peptide
      - round-trip distance ||z_original - encode_mu(decoded_peptide)||
      - cosine similarity between z_original and re-encoded decoded peptide
    """
    model.eval()
    _ensure_dir(out_dir)

    rows = []
    for z_index in range(Z_cand.size(0)):
        z = Z_cand[z_index:z_index + 1]
        x_dummy = torch.zeros((1, SEQ_LEN, VOCAB), device=Z_cand.device)
        logits, _ = model.dec(z, x_dummy)
        logits_2d = logits[0] / temperature
        probs = torch.softmax(logits_2d, dim=-1)

        # 1) deterministic argmax decode
        pep, mean_prob, min_prob, token_probs = _decode_logits_with_token_confidence(logits_2d)
        rt_l2, rt_cos = compute_roundtrip_latent_distance(model, pep, z[0])
        rows.append({
            "bo_iter": bo_iter,
            "z_candidate_index": z_index,
            "decode_rank": 0,
            "decode_type": "argmax",
            "peptide": pep,
            ...
            "z_norm": float(torch.norm(z[0], p=2).item()),
        })

        # 2) stochastic samples from the same optimized latent point
        for sample_rank in range(1, n_samples_per_z):
            sampled_idx = torch.multinomial(probs, num_samples=1).squeeze(-1)
            pep, mean_prob, min_prob, token_probs = _decode_logits_with_token_confidence(
                logits_2d, token_idx=sampled_idx
            )
            rt_l2, rt_cos = compute_roundtrip_latent_distance(model, pep, z[0])
            rows.append({
                "bo_iter": bo_iter,
                "z_candidate_index": z_index,
                "decode_rank": sample_rank,
                "decode_type": "sample",
                "peptide": pep,
                "decoder_confidence_mean": mean_prob,
                "decoder_confidence_min": min_prob,
    ...
    csv_path = os.path.join(out_dir, f"decoder_diagnostics_bo_iter_{bo_iter:03d}.csv")
    df.to_csv(csv_path, index=False)

    # Unique peptide list in the same order expected by existing filtering code
    unique_peptides = list(dict.fromkeys(df["peptide"].astype(str).tolist()))

    return unique_peptides, df
...
def plot_decoder_diagnostics(
    ...
    """Save plots for decoder confidence and round-trip consistency."""
    _ensure_dir(out_dir)
    paths: Dict[str, str] = {}
    ...
    # Scatter: decoder confidence vs round-trip distance
    plt.figure(figsize=(7, 5))
    plt.scatter(df["decoder_confidence_mean"], df["roundtrip_l2"], s=28, alpha=0.75)
    plt.xlabel("Decoder confidence: mean selected-token probability")
    plt.ylabel("Round-trip distance ||z - encoder_mu(decoded peptide)||")
    plt.title(f"BO iter {bo_iter}: decoder confidence vs round-trip distance")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    path = os.path.join(out_dir, f"{prefix}_confidence_vs_roundtrip_bo_iter_{bo_iter:03d}.png")
    plt.savefig(path, dpi=200)
    plt.close()
    paths["confidence_vs_roundtrip"] = path

    # Histogram: decoder confidence
    plt.figure(figsize=(7, 4))
    plt.hist(df["decoder_confidence_mean"], bins=min(20, max(5, len(df))))
    plt.xlabel("Decoder confidence")
    plt.ylabel("Decoded peptide count")
    plt.title(f"BO iter {bo_iter}: decoder confidence distribution")
    plt.tight_layout()
    path = os.path.join(out_dir, f"{prefix}_confidence_hist_bo_iter_{bo_iter:03d}.png")
    plt.savefig(path, dpi=200)
    plt.close()
    paths["confidence_hist"] = path

    # Histogram: round-trip distance
    plt.figure(figsize=(7, 4))
    plt.hist(df["roundtrip_l2"], bins=min(20, max(5, len(df))))
    plt.xlabel("Round-trip L2 distance")
    plt.ylabel("Decoded peptide count")
    plt.title(f"BO iter {bo_iter}: round-trip distance distribution")
    plt.tight_layout()
    path = os.path.join(out_dir, f"{prefix}_roundtrip_hist_bo_iter_{bo_iter:03d}.png")
    plt.savefig(path, dpi=200)
    plt.close()
    paths["roundtrip_hist"] = path

    return paths


@torch.no_grad()
def local_decoding_stability_test(
    model: FlowGRUVAE,
    Z_cand: torch.Tensor,
    bo_iter: int,
    out_dir: str = DECODER_MONITOR_DIR,
    max_z: int = LOCAL_DECODING_MAX_Z,
    n_neighbors: int = LOCAL_DECODING_N_NEIGHBORS,
    noise_std: float = LOCAL_DECODING_NOISE_STD,
    temperature: float = DECODER_TEMPERATURE,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Local decoding test:
      for up to max_z optimized latent points, decode n_neighbors perturbed points:
          z_local = z_original + noise_std * epsilon

    Saves:
      - raw local-neighbor decoding CSV
      - per-Z summary CSV
      - plots for local uniqueness and confidence/round-trip behavior
    """
    model.eval()
    _ensure_dir(out_dir)

    rows = []
    n_test = min(max_z, Z_cand.size(0))

    for z_index in range(n_test):
        z0 = Z_cand[z_index:z_index + 1]

        for neighbor_index in range(n_neighbors):
            noise = torch.randn_like(z0) * noise_std
            z_local = z0 + noise

            x_dummy = torch.zeros((1, SEQ_LEN, VOCAB), device=Z_cand.device)
            logits, _ = model.dec(z_local, x_dummy)
            logits_2d = logits[0] / temperature

            pep, mean_prob, min_prob, token_probs = _decode_logits_with_token_confidence(logits_2d)
            rt_l2_local, rt_cos_local = compute_roundtrip_latent_distance(model, pep, z_local[0])
            rt_l2_center, rt_cos_center = compute_roundtrip_latent_distance(model, pep, z0[0])
            ...
            rows.append({
                ...
                "noise_std": noise_std,
                "noise_l2": float(torch.norm(noise[0], p=2).item()),
                "peptide": pep,
                "decoder_confidence_mean": mean_prob,
                "decoder_confidence_min": min_prob,
                "token_probabilities": "|".join(f"{v:.6f}" for v in token_probs),
                "roundtrip_l2_to_local_z": rt_l2_local,
                "roundtrip_cosine_to_local_z": rt_cos_local,
                "roundtrip_l2_to_center_z": rt_l2_center,
                "roundtrip_cosine_to_center_z": rt_cos_center,
            })

    local_df = pd.DataFrame(rows)
    local_csv = os.path.join(out_dir, f"local_decoding_bo_iter_{bo_iter:03d}.csv")
    local_df.to_csv(local_csv, index=False)

    summary_rows = []
    if len(local_df) > 0:
        for z_index, g in local_df.groupby("z_candidate_index"):
            summary_rows.append({
                "bo_iter": bo_iter,
                "z_candidate_index": int(z_index),
                "n_neighbors": int(len(g)),
                "n_unique_peptides": int(g["peptide"].nunique()),
                "mean_decoder_confidence": float(g["decoder_confidence_mean"].mean()),
                "std_decoder_confidence": float(g["decoder_confidence_mean"].std(ddof=0)),
                "mean_roundtrip_l2_to_local_z": float(g["roundtrip_l2_to_local_z"].mean()),
                "mean_roundtrip_l2_to_center_z": float(g["roundtrip_l2_to_center_z"].mean()),
            })

    summary_df = pd.DataFrame(summary_rows)
    summary_csv = os.path.join(out_dir, f"local_decoding_summary_bo_iter_{bo_iter:03d}.csv")
    summary_df.to_csv(summary_csv, index=False)

    # Plot local uniqueness per optimized latent point
    if len(summary_df) > 0:
        plt.figure(figsize=(7, 4))
        plt.bar(summary_df["z_candidate_index"].astype(str), summary_df["n_unique_peptides"])
        plt.xlabel("Optimized latent candidate index")
        plt.ylabel("Unique peptides among local decodes")
        plt.title(f"BO iter {bo_iter}: local decoding diversity")
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"local_unique_peptides_bo_iter_{bo_iter:03d}.png"), dpi=200)
        plt.close()

    # Plot local noise size vs decoder confidence
    if len(local_df) > 0:
        plt.figure(figsize=(7, 5))
        plt.scatter(local_df["noise_l2"], local_df["decoder_confidence_mean"], s=28, alpha=0.75)
        plt.xlabel("Local perturbation size ||noise||")
        plt.ylabel("Decoder confidence")
        plt.title(f"BO iter {bo_iter}: local decoding confidence")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"local_noise_vs_confidence_bo_iter_{bo_iter:03d}.png"), dpi=200)
        plt.close()

        plt.figure(figsize=(7, 5))
        plt.scatter(local_df["noise_l2"], local_df["roundtrip_l2_to_local_z"], s=28, alpha=0.75)
        plt.xlabel("Local perturbation size ||noise||")
        plt.ylabel("Round-trip distance to local z")
        plt.title(f"BO iter {bo_iter}: local round-trip consistency")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"local_noise_vs_roundtrip_bo_iter_{bo_iter:03d}.png"), dpi=200)
        plt.close()

    return local_df, summary_df
...
def main():
    ...
    for bo_iter in range(BO_ITERS):
        ...
        cand_peps_raw, decoder_diag_df = decode_candidates_with_diagnostics(
            model,
            Z_cand,
            bo_iter=bo_iter,
            out_dir=DECODER_MONITOR_DIR,
            n_samples_per_z=DECODER_SAMPLES_PER_Z,
            temperature=DECODER_TEMPERATURE,
        )
        plot_decoder_diagnostics(decoder_diag_df, bo_iter=bo_iter, out_dir=DECODER_MONITOR_DIR)

        local_decoding_df, local_decoding_summary_df = local_decoding_stability_test(
            model,
            Z_cand,
            bo_iter=bo_iter,
            out_dir=DECODER_MONITOR_DIR,
            max_z=LOCAL_DECODING_MAX_Z,
            n_neighbors=LOCAL_DECODING_N_NEIGHBORS,
            noise_std=LOCAL_DECODING_NOISE_STD,
            temperature=DECODER_TEMPERATURE,
        )
        ...
        # Save accepted/scored decoder diagnostics for the peptides that entered black-box evaluation.
        accepted_diag_df = decoder_diag_df[decoder_diag_df["peptide"].isin(cand_peps)].copy()
        accepted_diag_df["accepted_for_blackbox"] = True
        accepted_diag_df["rejected_reason"] = ""
        score_map = {
            pep: [float(v) for v in y.detach().cpu().tolist()]
            for pep, y in zip(cand_peps, Y_new)
        }
        for obj_i, obj_name in enumerate(_OBJ_COLS):
            accepted_diag_df[obj_name] = accepted_diag_df["peptide"].map(
                lambda p: score_map.get(p, [np.nan] * len(_OBJ_COLS))[obj_i]
            )
        accepted_diag_df.to_csv(
            os.path.join(DECODER_MONITOR_DIR, f"accepted_decoder_diagnostics_scored_bo_iter_{bo_iter:03d}.csv"),
            index=False,
        )

        rejected_reason_map = {pep: reason for pep, reason in rejected}
        all_decode_status_df = decoder_diag_df.copy()
        all_decode_status_df["accepted_for_blackbox"] = all_decode_status_df["peptide"].isin(cand_peps)
        all_decode_status_df["rejected_reason"] = all_decode_status_df["peptide"].map(
            lambda p: "" if p in cand_peps else rejected_reason_map.get(p, "duplicate_raw_decode_or_not_selected")
        )
        all_decode_status_df.to_csv(
            os.path.join(DECODER_MONITOR_DIR, f"all_decoder_diagnostics_status_bo_iter_{bo_iter:03d}.csv"),
            index=False,
        )

        # Lightweight scalar logging for decoder reliability diagnostics.
        wandb.log({
            "bo_iter": bo_iter,
            "decoder/n_raw_decoded": int(len(decoder_diag_df)),
            "decoder/n_unique_decoded": int(len(cand_peps_raw)),
            "decoder/n_accepted_blackbox": int(len(cand_peps)),
            "decoder/confidence_mean": float(decoder_diag_df["decoder_confidence_mean"].mean()) if len(decoder_diag_df) else float("nan"),
            "decoder/roundtrip_l2_mean": float(decoder_diag_df["roundtrip_l2"].mean()) if len(decoder_diag_df) else float("nan"),
            "decoder/local_unique_peptides_mean": float(local_decoding_summary_df["n_unique_peptides"].mean()) if len(local_decoding_summary_df) else float("nan"),
            "decoder/local_roundtrip_l2_mean": float(local_decoding_df["roundtrip_l2_to_local_z"].mean()) if len(local_decoding_df) else float("nan"),
        })
