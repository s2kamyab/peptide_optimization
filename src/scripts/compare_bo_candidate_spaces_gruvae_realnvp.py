...
for the Cu-finetuned RealNVP model.
...
3) realnvp_zK
   64-D post-flow RealNVP coordinate:
       zK = RealNVP(h0)

4) decoder_mu_back [optional; requires .pt checkpoint]
   Actual 64-D GRU decoder latent obtained by:
       zK -> RealNVP inverse -> h0_back -> unstandardize -> mu_back

5) decoder_initial_hidden [diagnostic only; requires .pt checkpoint]
   128-D initial GRU hidden state generated by z_to_h(mu_back).
   This is NOT recommended as a BO search space.

The script evaluates:
A. Geometry / intrinsic dimensionality
B. Local objective smoothness
C. Held-out GP learnability
D. Offline multi-objective ParEGO on the same evaluated candidate pool

The training-history file can be supplied to verify that the checkpoint matches
the minimum validation flow-NLL epoch.
...
def load_direct_spaces(df: pd.DataFrame) -> Dict[str, np.ndarray]:
    ...
    zk_cols = sorted_prefixed_columns(df, "zK_")

    print(
        "Detected coordinate columns: "
        f"mu={len(mu_cols)}, h0={len(h_cols)}, zK={len(zk_cols)}"
    )
    ...
    if zk_cols:
        spaces["realnvp_zK"] = df[zk_cols].to_numpy(dtype=np.float64)
...
def derive_decoder_spaces(
    ...
    module = import_module_py314_safe(
        finetune_script,
        "cu_gruvae_realnvp_module_for_space_comparison",
    )
    ckpt = torch.load(checkpoint_path, map_location=device)

    cfg = module.ModelConfig(**ckpt["model_config"])
    flow_cfg = module.RealNVPConfig(**ckpt["flow_config"])

    vae = module.GRUVAE(cfg).to(device)
    flow = module.RealNVPFlow(cfg.latent_dim, flow_cfg).to(device)
    ...
    zk_cols = sorted_prefixed_columns(df, "zK_")
    if not zk_cols:
        raise ValueError("Coordinate CSV has no zK_00 ... zK_63 columns.")

    zk_all = torch.tensor(
        df[zk_cols].to_numpy(dtype=np.float32),
        device=device,
    )
    ...
    for start in range(0, len(df), batch_size):
        zK = zk_all[start:start + batch_size]
        h_back, _ = flow.inverse(zK)
        mu_back = h_back * latent_std.unsqueeze(0) + latent_mean.unsqueeze(0)
        initial_hidden = vae.dec.z_to_h(mu_back)

        h0_back_all.append(h_back.detach().cpu().numpy())
        mu_back_all.append(mu_back.detach().cpu().numpy())
        hidden_all.append(initial_hidden.detach().cpu().numpy())

    return {
        "realnvp_inverse_h0": np.concatenate(h0_back_all, axis=0).astype(np.float64),
...
def inspect_realnvp_training_history(history_csv: str):
    if not history_csv:
        return None
    ...
    for col, direction in [
        ...
        result[col] = {
            "epoch": int(hist.loc[idx, "epoch"]),
            "value": float(s.loc[idx]),
        }
    return result


def verify_checkpoint_against_history(
    checkpoint_path: str,
    training_history: str,
    allow_mismatch: bool,
):
    selection = inspect_realnvp_training_history(training_history)
    if not selection or "val_flow_nll" not in selection:
        print("WARNING: Could not determine best val_flow_nll epoch from history.")
        return selection

    ckpt = torch.load(checkpoint_path, map_location="cpu")
    ckpt_epoch = int(ckpt.get("epoch", -1))
    best_epoch = int(selection["val_flow_nll"]["epoch"])

    print(
        f"RealNVP history best val_flow_nll: epoch={best_epoch}, "
        f"value={selection['val_flow_nll']['value']:.8f}"
    )
    print(
        f"Supplied checkpoint: epoch={ckpt_epoch}, "
        f"selection={ckpt.get('checkpoint_selection', 'unknown')}"
    )

    if ckpt_epoch != best_epoch:
        msg = (
            f"Checkpoint epoch {ckpt_epoch} does not match best "
            f"val_flow_nll epoch {best_epoch}."
        )
        if allow_mismatch:
            print("WARNING:", msg)
        else:
            raise RuntimeError(
                msg + " Use the best_val_flow_nll checkpoint or pass "
                "--allow-checkpoint-epoch-mismatch."
            )
    return selection
...
def main():
    p = argparse.ArgumentParser(
        description="Compare GRU-VAE and RealNVP coordinate spaces for peptide BO."
    )
    ...
    p.add_argument(
        "--checkpoint",
        default="",
        help="Optional RealNVP .pt checkpoint; normally best_val_flow_nll_*.pt.",
    )
    p.add_argument(
        "--training-history",
        default="",
        help="Optional RealNVP training history used to verify the best flow-NLL epoch.",
    )
    p.add_argument(
        "--allow-checkpoint-epoch-mismatch",
        action="store_true",
    ...
    p.add_argument(
        "--finetune-script",
        default="",
        help="RealNVP fine-tuning .py defining GRUVAE, RealNVPConfig, and RealNVPFlow.",
    )
    ...
    if args.training_history:
        sel = inspect_realnvp_training_history(args.training_history)
        if sel:
            print("Training-history selections:")
            for metric, info in sel.items():
                print(f"  {metric}: epoch={info['epoch']} value={info['value']:.8f}")

    if args.checkpoint and args.training_history:
        verify_checkpoint_against_history(
            args.checkpoint,
            args.training_history,
            args.allow_checkpoint_epoch_mismatch,
        )
    ...
    expected_dim = 64
    for core_name in ["encoder_mu", "encoder_h0", "realnvp_zK", "realnvp_inverse_h0", "decoder_mu_back"]:
    ...
    lines.append(
        "realnvp_zK is preferable only if it improves objective learnability and/or offline BO sample efficiency over encoder_mu / encoder_h0."
    )
    lines.append(
    ...
    lines.append(
        "RealNVP zK is analytically invertible to h0 and therefore has no DDIM-style inversion approximation error."
    )
    lines.append(
