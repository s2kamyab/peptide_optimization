...
1. The fine-tuning framework defaults to
   ...
   Therefore this BO code operates in the native, non-spherical epsilon space.
   It does NOT normalize epsilon to radius sqrt(64).
...
def run(args):
    ...
    if (
        projected_to_sphere
        and not args.allow_sphere_projected_checkpoint
    ):
        raise RuntimeError(
            "The supplied checkpoint was trained/exported with sphere-projected "
            "epsilon, but this matched BO configuration is for native epsilon. "
            "Use a non-projected checkpoint, or deliberately pass "
            "--allow-sphere-projected-checkpoint and use the older spherical BO "
            "implementation."
        )
    ...
    # Native, NON-SPHERICAL epsilon coordinates.
    ...
    print(
        "Native epsilon is NOT sphere-normalized."
    )
    print(
    ...
    run_config.update({
        ...
        "checkpoint_metrics":
            ckpt.get("metrics", {}),
        "bo_space":
            "native_non_spherical_diffusion_epsilon",
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Leakage-safe multi-objective GP/qLogEHVI BO in native "
            "non-spherical latent-diffusion epsilon space."
        )
    )
    ...
    p.add_argument(
        "--allow-sphere-projected-checkpoint",
        action="store_true",
        help=(
            "Not recommended for the attached history. The matched experiment "
            "uses native non-spherical epsilon."
        ),
    )
    ...
    # Native epsilon geometry.
    p.add_argument(
        "--epsilon-bound",
        type=float,
        default=4.0,
        help=(
            "Initial symmetric coordinate bound for native epsilon. "
            "Auto-expanded before BO to contain all Cu TRAIN epsilon coordinates."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        default=0.75,
        help=(
            "Trust-region half-width in raw native epsilon coordinate units."
        ),
    )
    ...
    p.add_argument(
        ...
        help=(
            "Gaussian perturbation std in native non-spherical epsilon coordinates."
        ),
    )
    p.add_argument(
