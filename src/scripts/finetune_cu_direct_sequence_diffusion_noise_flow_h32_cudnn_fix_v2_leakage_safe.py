...
#
# Leakage control added in this revision only:
#   Cu peptides inherit pretraining split labels by exact sequence.
#   Exact peptides occurring in >1 pretraining split are excluded from all
#   fine-tuning partitions. No random fine-tuning split fallback is used.
#
# BO export:
...
# ============================================================
# Leakage-safe split reuse from pretraining parts
# ============================================================
...
def discover_pretraining_parts(parts_dir: str, part_glob: str) -> List[str]:
    ...
    files = [
        f for f in files
        if "failures" not in os.path.basename(f).lower()
    ]
    if not files:
        raise FileNotFoundError(
            f"No pretraining part files found in {parts_dir!r} "
            f"using pattern {part_glob!r}"
        )
    return files
...
def build_pretraining_peptide_split_map(
    ...
    """
    Build a global exact-peptide -> split map from the original pretraining parts.

    Leakage rule:
      * exact peptide occurs in one unique split -> retain that split
      * exact peptide occurs in multiple splits -> exclude it from fine-tuning
        train/validation/test entirely

    This prevents exact-sequence train/validation/test leakage while leaving the
    original H32 fine-tuning framework unchanged.
    """
    ...
    print(
        "[SPLIT-MAP] safe unique peptide mappings="
        f"{len(split_map)}; excluded exact cross-split conflicts="
        f"{len(conflicts_df)}"
    )
...
def load_cu_dataset(
    ...
    """
    Load Cu data while inheriting exact-sequence split assignments from
    pretraining. No random fine-tuning split is created.

    All non-leakage-related v2 behavior is preserved.
    """
    ...
    if args.score_col not in df.columns:
        ...
        print(
            f"[WARN] score column {args.score_col!r} not found. "
            "Computing cu_proxy_score from peptide and labels."
        )
        if args.labels_col not in df.columns:
            raise KeyError(
                f"Cannot compute proxy score because labels column "
                f"{args.labels_col!r} is missing."
            )
        df["cu_proxy_score"] = [
    ...
    for idx, row in df.iterrows():
        ...
        split = split_map.get(pep)
        if split is None:
            dropped_rows.append(int(idx))
            continue

        peptides.append(pep)
        scores.append(float(score_raw))
        splits.append(str(split).strip().lower())
        kept_rows.append(int(idx))

    if dropped_rows:
        dropped_path = os.path.join(
            args.out_dir,
            "cu_peptides_unmapped_or_cross_split_conflicted.csv",
        )
        df.iloc[dropped_rows].copy().to_csv(dropped_path, index=False)
        print(
            f"[LEAKAGE-SAFETY] Dropped {len(dropped_rows)} Cu rows whose peptide "
            "was absent from the safe pretraining split map or occurred in "
            f"multiple pretraining splits. Saved: {dropped_path}"
        )

    if not peptides:
        raise ValueError(
            f"No valid leakage-safe Cu peptides remain in {args.cu_csv}"
        )
    ...
    if unexpected:
        raise RuntimeError(
            f"Unexpected split labels inherited from pretraining: {unexpected}"
        )
    ...
    split_sets = {
        s: set(
            kept_df.loc[
                kept_df["_split_used"].astype(str).str.lower().eq(s),
                "_clean_peptide",
            ].tolist()
        )
        for s in [
    ...
    if exact_train_val or exact_train_test or exact_val_test:
        raise RuntimeError(
            "Exact peptide leakage remains after split-map filtering: "
            f"train-val={exact_train_val}, "
            f"train-test={exact_train_test}, "
            f"validation-test={exact_val_test}"
        )

    counts = kept_df["_split_used"].value_counts().to_dict()
    for split_name in allowed:
        if counts.get(split_name, 0) == 0:
            raise RuntimeError(
                f"No Cu peptides remain in required split {split_name!r}"
            )

    audit = {
        "source_cu_rows": int(len(df)),
        "kept_rows": int(len(kept_df)),
        "dropped_unmapped_or_cross_split_conflicted_rows":
            int(len(dropped_rows)),
        "split_source": "pretraining part CSV exact-peptide split map",
        "random_resplitting_used": False,
        "cu_csv_split_column_used_for_membership": False,
        ...
        "duplicate_cu_row_policy": "unchanged_from_v2_no_reaggregation",
    }
    with open(
...
def train(args: argparse.Namespace) -> str:
    ...
    split_audit = {
        ...
        "framework_settings_changed": False,
    }
    with open(
        os.path.join(
            args.out_dir,
    ...
    if len(split_conflicts_df):
        print(
            ...
            "sequences occurring in more than one pretraining split."
        )
        print(
    ...
    if len(test_idx) == 0:
        raise ValueError(
            "Untouched Cu test split is empty; refusing to reuse validation as test."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--pretraining-parts-dir",
        required=True,
        help=(
            "Directory containing the original pretraining part CSVs. "
            "Used only to inherit leakage-safe exact-peptide split membership."
        ),
    )
    p.add_argument(
