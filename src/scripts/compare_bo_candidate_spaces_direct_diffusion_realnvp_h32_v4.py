...
"""
Compare BO candidate spaces exported by the H32 direct sequence diffusion +
RealNVP Cu fine-tuning framework.

Matched fine-tuning framework
-----------------------------
The attached H32 fine-tuner does NOT export deterministic DDIM-inverted epsilon0
or a PCA-whitened z_bo coordinate. Instead it uses:

    peptide x0
      -> optimized spherical epsilon preimage epsilon0
      -> RealNVP flow
      -> epsilonK
      -> deterministic DDIM
      -> peptide
...
Spaces compared
---------------
1) optimized_epsilon0
   200-D optimized diffusion-noise preimage before RealNVP. The fine-tuner
   constructs this coordinate by direct gradient optimization against peptide
   reconstruction while projecting to the radius sqrt(200) sphere.

2) realnvp_epsilonK
   200-D Cu-aligned coordinate after the trained RealNVP transformation:
       epsilon0 -> RealNVP -> epsilonK

The H32 fine-tuner exports only epsilon0_* and epsilonK_*; therefore no PCA/z_bo
space is expected or compared.

Split policy
------------
By default, only the exported `train` rows are used for candidate-space
selection so validation/test remain held out. If the fine-tuning run itself was
made leakage-safe, this script preserves that exported split membership.

Objective support
-----------------
The H32 fine-tuner trains a scalar ScoreHead against `final_score` and exports
the target as `score`. Therefore the default comparison is scalar GP
learnability + offline GP expected-improvement BO. Multiple objective columns
may still be supplied for a post-hoc multi-objective benchmark if they are
present in the coordinate CSV or merged externally.
...
def load_direct_spaces(df: pd.DataFrame) -> Dict[str, np.ndarray]:
    """Load the two coordinate systems exported by the H32 fine-tuner."""
    spaces = {}
    ...
    print(
        ...
        f"epsilon0={len(eps0_cols)}, epsilonK={len(epsk_cols)}"
    )

    if eps0_cols:
        spaces["optimized_epsilon0"] = df[eps0_cols].to_numpy(dtype=np.float64)
    if epsk_cols:
...
# ---------------------------------------------------------------------
# H32 spherical-coordinate diagnostics
# ---------------------------------------------------------------------

def spherical_metrics(X: np.ndarray, target_radius: float = math.sqrt(200.0)) -> Dict[str, float]:
    norms = np.linalg.norm(np.asarray(X, dtype=float), axis=1)
    return {
        "norm_mean": float(np.mean(norms)),
        "norm_std": float(np.std(norms)),
        "norm_min": float(np.min(norms)),
        "norm_max": float(np.max(norms)),
        "target_radius_sqrt200": float(target_radius),
        "mean_abs_radius_error": float(np.mean(np.abs(norms - target_radius))),
        "max_abs_radius_error": float(np.max(np.abs(norms - target_radius))),
    }
...
def main():
    p = argparse.ArgumentParser(
        description=(
            "Compare H32 optimized epsilon0 preimages and RealNVP epsilonK spaces "
    ...
    if not spaces:
        raise ValueError(
            "No H32 direct-diffusion coordinate columns found. Expected "
            "epsilon0_* and/or epsilonK_*."
        )

    # Dimensional sanity checks matching the attached fine-tuner.
    if "optimized_epsilon0" in spaces and spaces["optimized_epsilon0"].shape[1] != 200:
        raise ValueError(
            f"optimized_epsilon0 has {spaces['optimized_epsilon0'].shape[1]} dimensions; expected 200."
        )
    ...
    lines = [
        "H32 DIRECT-DIFFUSION + REALNVP BO CANDIDATE-SPACE COMPARISON",
        "=" * 88,
        f"n_rows={len(Y)}",
        ...
        "optimized_epsilon0 is the H32 fine-tuner's 200-D optimized spherical preimage baseline.",
        ...
        "No PCA/z_bo coordinate exists in this H32 framework and none is introduced by this comparison.",
