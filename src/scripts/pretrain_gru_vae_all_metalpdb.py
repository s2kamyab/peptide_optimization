...
def find_pretrain_files(parts_dir: str) -> List[str]:
    pattern = os.path.join(parts_dir, "metalpdb_binding_windows_len10_part_*.csv")
...
def train(args: argparse.Namespace) -> str:
    ...
    out_path = os.path.join(args.out_dir, "pretrained_gru_vae_no_flow.pt")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Pretrain GRU-VAE on all-metal MetalPDB length-10 peptide windows.")
    p.add_argument("--parts-dir", default="metalpdb_all_metals_unique/binding_windows_len10/parts")
    p.add_argument("--peptide-col", default="peptide_len10")
    p.add_argument("--out-dir", default="transfer_gru_vae_checkpoints")
