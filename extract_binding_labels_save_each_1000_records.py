...
OUT_CSV  = "metalpdb_binding_windows_len10.csv"
...
def save_batch(rows, out_csv_path, is_first_batch):
    """
    Save a batch of rows to CSV.
    If is_first_batch=True, create new file with header.
    ...
    if is_first_batch:
        ...
        batch_df.to_csv(out_csv_path, index=False, mode='w')
    else:
        # Append without header
        batch_df.to_csv(out_csv_path, index=False, mode='a', header=False)
    
    print(f"  → Saved batch of {len(rows)} rows (total so far)")
...
def main():
    ...
    for i, r in df.iterrows():
        ...
        for chain_id, bind_resnums in bind_map.items():
            ...
            for bind_resseq in sorted(bind_resnums):
                ...
                # Save batch every BATCH_SIZE records
                if len(rows) >= BATCH_SIZE:
                    save_batch(rows, OUT_CSV, is_first_batch)
                    batch_count += 1
                    rows = []  # Clear the list
    ...
    print(f"\nCompleted! Saved {batch_count} batches to: {OUT_CSV}")
