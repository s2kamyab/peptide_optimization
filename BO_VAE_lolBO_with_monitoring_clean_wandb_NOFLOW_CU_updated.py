# ============================================================
# Semi-supervised GRU-VAE + Deep-kernel SVGP
...
DATA_CSV = "metalpdb_binding_windows_len10_MN_scored_ranked.csv"
...
# ============================================================
# Monitoring utilities (ported from train_seqVAE_one_hot_masked.py)
...
@torch.no_grad()
def plot_latent_distributions(mu, logvar, z, out_dir: str, name: str):
    ...
    _hist(z.detach().cpu().numpy().ravel(), "z", f"{name}_z_hist.png")

    zn = z.norm(dim=1).detach().cpu().numpy()
    _hist(zn, "||z||", f"{name}_z_norm_hist.png")

@torch.no_grad()
def plot_flow_diagnostics(*args, **kwargs):
    return
...
class GRUVAE(nn.Module):
    ...
    def forward(self, x_onehot: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        z = self.reparam(mu, logvar)                         # [B,d]
        logits, dec_out_top = self.dec(z, x_onehot)         # [B,L,V], [B,L,H]
        return {
            "mu": mu, "logvar": logvar,
            "z": z,
            "logits": logits,
...
# ============================================================
# Deep-kernel SVGP: GP consumes latent features from encoder (we use mu)
...
#             # We use mu as the latent feature for the deep kernel.
...
def train_one_epoch_joint(
    ...
    """
    Minibatch over X.
    - For ALL items in batch: VAE ELBO
    - For labeled subset within batch: SVGP variational ELBO
    Validation is NOT run inside the minibatch loop.
    """
    model.train()
    gp.train()
    likelihood.train()
    ...
    for start in range(0, n_all, batch_size):
        ...
        # ---- labeled items in this minibatch ----
        idx_cpu = idx.detach().cpu().tolist()
        lab_globals = [g for g in idx_cpu if g in labeled_set]
        ...
        print(
            global_step,
            f"loss: {loss.item():.4f} "
            f"(recon: {recon.item():.4f}, kl: {kl.item():.4f}, svgp: {loss_svgp.item():.4f})"
        )

    return global_step

@torch.no_grad()
def encode_to_latent_mu(model: GRUVAE, X: torch.Tensor) -> torch.Tensor:
    model.eval()
...
def evaluate_vae_on_indices(
    ...
    for start in range(0, n, batch_size):
        ...
        sum_recon += float(recon.detach().cpu())
        sum_kl += float(kl.detach().cpu())
        token_acc_sums += np.asarray(
            compute_token_accuracy_per_position(out["logits"], target),
            dtype=np.float64,
        )
        n_batches += 1
    ...
    mean_recon = sum_recon / max(1, n_batches)
    mean_kl = sum_kl / max(1, n_batches)
    vae_loss = mean_recon + 0.01 * mean_kl
    mean_pos_acc = (token_acc_sums / max(1, n_batches)).tolist()
    token_acc = float(np.mean(mean_pos_acc))
...
def main():
    ...
    train_pareto_save_df.to_csv("bo_results/train_pareto.csv", index=False)
    print(f"Training Pareto: {len(train_pareto_peptides)} solutions -> bo_results/train_pareto.csv")
    ...
    # ---- Initialize GRU-VAE ----
    model = GRUVAE(hidden_size=H, latent_dim=LATENT_DIM, n_layers=N_GRU_LAYERS).to(DEVICE)
    ...
    final_bo_pareto_df.to_csv("bo_results/bo_final_pareto_noflow_CU_updated.csv", index=False)
    print(f"Final BO Pareto: {len(final_bo_pareto_df)} solutions -> bo_results/bo_final_pareto_noflow_CU_updated.csv")
    ...
    if len(final_bo_pareto_df) > 0:
        ...
        novel_dom_df.to_csv("bo_results/bo_pareto_dominates_train_noflow_CU_updated.csv", index=False)
        n_dominating = int(sum(dom_train_mask))
        print(f"BO solutions dominating training Pareto: {n_dominating} "
              f"-> bo_results/bo_pareto_dominates_train_noflow_CU_updated.csv")
    else:
        n_dominating = 0
