...
from botorch.utils.multi_objective.box_decompositions import NondominatedPartitioning, FastNondominatedPartitioning
from botorch.sampling.normal import SobolQMCNormalSampler
from botorch.optim import optimize_acqf
from botorch.models.transforms.outcome import Standardize

from black_box_fcn_mo_gold_updated import blackbox_fc
# ============================================================
# Config
# ============================================================
...
DATA_CSV = "metalpdb_binding_windows_len10_AU_scored_ranked.csv"
...
def pareto_mask_maximize(Y: torch.Tensor) -> torch.Tensor:
    ...
    Y: [N, M] — computed on CPU to avoid O(N²) GPU memory allocation.
    """
    if Y.numel() == 0:
    ...
    return (~dominated).to(orig_device)
...
def train_one_epoch_joint(
    ...
    if train_indices is None:
        train_indices = torch.arange(X_all.size(0))
    else:
        train_indices = train_indices.cpu()

    n_all = train_indices.numel()
    perm = train_indices[torch.randperm(n_all)]
    ...
    for start in range(0, n_all, batch_size):
        ...
        xb = X_all[idx].to(DEVICE)  # move batch to GPU here
        ...
        loss_svgp = torch.tensor(0.0, device=DEVICE)
        if len(lab_globals) > 0:
            lab_globals_t = torch.tensor(lab_globals)
            lab_local = torch.tensor([labeled_pos[g] for g in lab_globals])
            yb = y_labeled[lab_local].to(DEVICE)  # [B_lab]

            # Deep kernel: latent features from encoder.
            mu_lab, _, _, _ = model.enc(X_all[lab_globals_t].to(DEVICE))  # [B_lab,d]
...
def nondominated_mask(Y: torch.Tensor) -> torch.Tensor:
    """Y: [N, M], maximize all objectives. Runs on CPU to avoid GPU pressure."""
    orig_device = Y.device
    Y = Y.cpu()
    N = Y.size(0)
    is_nd = torch.ones(N, dtype=torch.bool)
    for i in range(N):
        if not is_nd[i]:
            continue
...
def main():
    ...
    # ---- Build X_all (kept on CPU; batches are moved to GPU on demand) ----
    X_all = onehot_encode_peptides(peptides)  # stays on CPU
    ...
    val_idx = torch.tensor(all_idx0[:n_val], dtype=torch.long)    # CPU
    train_idx = torch.tensor(all_idx0[n_val:], dtype=torch.long)  # CPU
    ...
    train_pareto_save_df.to_csv("bo_results/train_pareto_gold_updated.csv", index=False)
    print(f"Training Pareto: {len(train_pareto_peptides)} solutions -> bo_results/train_pareto_gold_updated.csv")
    ...
    # Y_lab from pre-loaded Y_train_full — kept on CPU; moved to GPU only when needed
    Y_lab = Y_train_full[torch.tensor(labeled_idx, dtype=torch.long)]
    ...
    with torch.no_grad():
        Z_init = encode_to_latent_mu(model, X_all[torch.tensor(labeled_idx)].to(DEVICE))
    ...
    ref_t = torch.tensor(ref_point, dtype=Y_lab.dtype)
    best_hv = float(Hypervolume(ref_point=ref_t).compute(Y_lab.cpu()))
    ...
    for bo_iter in range(BO_ITERS):
        ...
        X_lab = X_all[torch.tensor(labeled_idx)].to(DEVICE)
        Z_lab = encode_to_latent_mu(model, X_lab)  # [N_lab,d]

        # =========================================================
        # 2) Surrogate-only update (MO GP) on current data
        # =========================================================
        # Cap GP training data to avoid O(N²) memory blowup over BO iterations
        GP_MAX_POINTS = 512
        if Z_lab.size(0) > GP_MAX_POINTS:
            keep = torch.randperm(Z_lab.size(0))[:GP_MAX_POINTS]
            Z_lab_bo = Z_lab[keep].detach().double()
            Y_lab_bo = Y_lab[keep].detach().double().to(DEVICE)
        else:
            Z_lab_bo = Z_lab.detach().double()
        ...
        #     Y=Y_lab_bo
        # )
        ref_point_t = torch.tensor(ref_point, device=DEVICE, dtype=Y_lab_bo.dtype)
        partitioning = FastNondominatedPartitioning(
            ref_point=torch.tensor(ref_point_t, device=DEVICE),
            Y=Y_lab_bo,
        )
        ...
        Y_new = black_box_objectives(cand_peps)  # kept on CPU
        ...
        for pep in cand_peps:
            if pep in peptides:
                ...
                peptides.append(pep)
                X_all = torch.cat([X_all, onehot_encode_peptides([pep])], dim=0)  # stays CPU
                new_indices.append(len(peptides) - 1)

        labeled_idx.extend(new_indices)
        Y_lab = torch.cat([Y_lab, Y_new.cpu()], dim=0)  # stays CPU
        ...
        with torch.no_grad():
            ref_t = torch.tensor(ref_point, dtype=Y_lab.dtype)
            hv = float(Hypervolume(ref_point=ref_t).compute(Y_lab.cpu()))
    ...
    final_bo_pareto_df.to_csv("bo_results/bo_final_pareto_gold_updated.csv", index=False)
    print(f"Final BO Pareto: {len(final_bo_pareto_df)} solutions -> bo_results/bo_final_pareto_gold_updated.csv")
    ...
    if len(final_bo_pareto_df) > 0:
        ...
        novel_dom_df.to_csv("bo_results/bo_pareto_dominates_train_gold_updated.csv", index=False)
        n_dominating = int(sum(dom_train_mask))
        print(f"BO solutions dominating training Pareto: {n_dominating} "
              f"-> bo_results/bo_pareto_dominates_train_gold_updated.csv")
    else:
        n_dominating = 0
